...
const AVATAR_OPTIONS: { key: AvatarKey; label: string; icon: React.ReactNode }[] = [
  {
    key: "sprout",
    label: "Sprout",
    icon: <Sprout className="w-4 h-4 text-emerald-400" aria-hidden="true" />,
  },
  {
    key: "globe",
    label: "Globe",
    icon: <Globe className="w-4 h-4 text-blue-400" aria-hidden="true" />,
  },
  {
    key: "leaf",
    label: "Leaf",
    icon: <Leaf className="w-4 h-4 text-teal-400" aria-hidden="true" />,
  },
  {
    key: "bike",
    label: "Cycle",
    icon: <Bike className="w-4 h-4 text-amber-400" aria-hidden="true" />,
  },
];
...
const HOW_IT_WORKS = [
  {
    step: "01",
    icon: <Target className="w-5 h-5 text-emerald-400" aria-hidden="true" />,
    title: "Log your actions",
    body: "Complete daily challenges or record custom offsets across energy, transport, diet, and waste.",
  },
  {
    step: "02",
    icon: <Cpu className="w-5 h-5 text-emerald-400" aria-hidden="true" />,
    title: "Sync & automate",
    body: "Connect simulated smart-home and transport trackers that generate carbon credits passively.",
  },
  {
    step: "03",
    icon: <MessageSquare className="w-5 h-5 text-emerald-400" aria-hidden="true" />,
    title: "Get AI guidance",
    body: "Receive personalized insights and chat with your coach to keep improving week after week.",
  },
];
      ...
      <section className="grid lg:grid-cols-2 gap-10 items-center pt-4 md:pt-8">
        <div className="space-y-6 text-center lg:text-left animate-fade-in-up">
          ...
          <h2 className="font-display font-extrabold text-4xl md:text-5xl xl:text-6xl text-white tracking-tight leading-[1.05]">
            Understand, track, and <span className="text-emerald-400">cut your carbon</span>{" "}
            footprint.
          </h2>
          <p className="text-sm md:text-base text-slate-400 max-w-xl mx-auto lg:mx-0 leading-relaxed">
            CarbonSync turns everyday choices into measurable climate impact. Log simple actions,
            sync simulated smart-home and transport telemetry, and get personalized guidance from a
            conversational AI coach powered by Google Gemini.
          </p>
          ...
          <div className="flex items-center gap-5 justify-center lg:justify-start pt-3 text-[11px] text-slate-500">
            <span className="flex items-center gap-1.5">
              <ShieldCheck className="w-4 h-4 text-emerald-400" aria-hidden="true" /> Privacy-first
            </span>
            <span className="flex items-center gap-1.5">
              <Cpu className="w-4 h-4 text-emerald-400" aria-hidden="true" /> Gemini AI
            </span>
            <span className="flex items-center gap-1.5">
              <Globe className="w-4 h-4 text-emerald-400" aria-hidden="true" /> Local-first storage
            </span>
          </div>
        </div>
        ...
        <div className="glass rounded-3xl p-6 md:p-8 relative overflow-hidden animate-fade-in-scale">
          <div
            className="absolute -top-10 -right-10 w-40 h-40 bg-emerald-500/20 rounded-full blur-3xl pointer-events-none"
            aria-hidden="true"
          ></div>
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-[10px] font-mono text-emerald-300 bg-emerald-500/10 px-2.5 py-1 rounded-full border border-emerald-500/20">
              DEMO
            </span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {HERO_STATS.map((s) => (
              <div key={s.label} className="bg-white/5 border border-white/10 rounded-2xl p-4">
                ...
                <p
                  className={`font-display font-extrabold text-2xl ${s.color} font-mono leading-tight`}
                >
          ...
          <div className="mt-4 bg-emerald-500/5 border border-emerald-500/15 rounded-2xl p-3.5 flex items-start gap-2.5">
            <Sparkles
              className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5 animate-pulse"
              aria-hidden="true"
            />
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="glass rounded-2xl p-5 text-center">
            <p className="font-display font-extrabold text-2xl md:text-3xl text-emerald-400">
              {stat.value}
            </p>
      ...
      <section id="ai-demo" className="space-y-6">
        <div className="text-center max-w-2xl mx-auto space-y-3">
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            No sign-up required. Try the live conversational assistant below — it gives practical,
            context-aware advice and rough CO₂ estimates on demand.
          </p>
        </div>
      ...
      <section className="space-y-8">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="glass rounded-2xl p-6 hover:border-emerald-500/25 transition-all group"
            >
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {HOW_IT_WORKS.map((s, i) => (
            <div key={s.step} className="relative glass rounded-2xl p-6">
              <span className="font-display font-extrabold text-4xl text-white/10 absolute top-4 right-5">
                {s.step}
              </span>
              ...
              {i < 2 && (
                <ArrowRight
                  className="hidden md:block w-5 h-5 text-slate-700 absolute -right-3.5 top-1/2 -translate-y-1/2 z-10"
                  aria-hidden="true"
                />
              )}
            </div>
      ...
      <section id="get-started" className="grid lg:grid-cols-2 gap-8 items-center scroll-mt-24">
        <div className="space-y-5 text-center lg:text-left">
          ...
          <p className="text-sm text-slate-400 leading-relaxed max-w-md mx-auto lg:mx-0">
            Your progress, achievements, and carbon ledger are saved to a private local database on
            the server, secured behind your own password — so you can pick up right where you left
            off.
          </p>
          ...
          >
            ...
            {authMode === "register" && (
              <div className="space-y-1">
                <label
                  htmlFor="auth-name"
                  className="text-[11px] font-mono uppercase text-slate-400 font-semibold block"
                >
                  Full Name
                </label>
                <input
            ...
            {authError && (
              <p
                role="alert"
                className="text-[11px] text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 flex items-center gap-2"
              >
